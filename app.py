...
.kal-table th { background: linear-gradient(135deg, GRAD1 0%, GRAD2 100%); color: #fff; padding: 10px 4px; text-align: center; font-weight: 700; border-radius: 10px; font-size: clamp(11px, 1.2vw, 14px); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; border: clamp(3px, 0.4vw, 6px) solid transparent; background-clip: padding-box; }
...
.tv-title { color:#ffffff; font-size:3.5vw; font-weight:800; }
...
.tv-slogan { text-align:center; margin-top:2vh; color:#ffffff; font-size:3.5vw; font-weight:800; letter-spacing:1px; line-height:1.6; text-shadow:0 2px 8px rgba(0,0,0,0.5); }
    ...
    _slog_lines = [t.strip()[:50] for t in tv_slogan.replace("|", "\n").split("\n") if t.strip()][:2]
...
with st.expander("📺 Buat Link TV Masjid Anda (Solusi Multi-Masjid)"):
    ...
    with g_col1:
        ...
        st.markdown("**✍️ Tulisan statis bawah TV (maks 2 baris × 50 karakter):**")
        s_col1, s_col2 = st.columns(2)
        with s_col1:
            g_slogan1 = st.text_input("Baris 1", value="Selamat Menunaikan Ibadah Sholat", max_chars=50, key="g_slogan1")
        with s_col2:
            g_slogan2 = st.text_input("Baris 2", value="", max_chars=50, key="g_slogan2")
        g_slogan = "\n".join([t for t in [g_slogan1, g_slogan2] if t.strip()])
